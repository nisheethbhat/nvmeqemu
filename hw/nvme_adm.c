...
static uint32_t adm_check_cqid(NVMEState *n, uint16_t cqid)
{
	...
	printf("kw q: check if exists cqid %d \n", cqid);
...
static uint32_t adm_cmd_alloc_sq(NVMEState *n, NVMECmd *cmd, NVMECQE *cqe)
{
	...
	if (c->pc == 0) {
		printf("Non physicaly contiguous memory !\n"
			"Not supported yet\n");
	...
	printf("sq->id %d, sq->dma_addr 0x%x, %lu\n",
...
static uint32_t adm_cmd_del_cq(NVMEState *n, NVMECmd *cmd, NVMECQE *cqe)
{
	...
	if (cq->usage_cnt) {
		printf("Error. Some sq are still connected to CQ %d\n", c->qid);
		sf->sc = NVME_SC_INVALID_FIELD;
...
*/
	...
	if (c->pc == 0) {
		...
		printf("c->pc == 0 Not supported yet\n");
		sf->sc = NVME_SC_INVALID_FIELD;
		return FAIL;
	}

	if (c->qid == 0 || c->qid >= NVME_MAX_QID) {
		printf ("c->qid == 0 || c->qid >= NVME_MAX_QID\n");
		sf->sct = NVME_SCT_CMD_SPEC_ERR;
		sf->sc = NVME_INVALID_QUEUE_IDENTIFIER;
		return FAIL;
	}
	...
	/* Queue Size */
	if (c->qsize > n->ctrlcap.mqes) {
		printf("c->qsize %d, n->ctrlcap.mqes %d\n",
	...
	printf("kw q: cq[%d] phase_tag   %d\n", cq->id, cq->phase_tag);
	printf("kw q: msix vector. cq[%d] vector %d irq_enabled %d\n",
...
static uint32_t adm_cmd_identify(NVMEState *n, NVMECmd *cmd, NVMECQE *cqe)
{
	...
	if (c->prp2) {
		/* Don't like it! */
		printf("%s(): prp2 is set\n", __func__);
	}

	if (c->prp1 == 0) {
		/* Error!*/
		printf("%s(): prp1 is not set\n", __func__);
		sf->sc = NVME_SC_INVALID_OPCODE;
		return FAIL;
	}
...
/*5.1 Abort command
The Abort command is used to cancel/abort a specific I/O command previously issued to the Admin or an I/O Submission Queue.
Host software may have multiple Abort commands outstanding, subject to the constraints of the Abort Command Limit indicated in the Identify Controller data structure.
An abort is a best effort command; the command to abort may have already completed, currently be in execution, or may be deeply queued.
It is implementation specific if/when a controller chooses to complete the command with an error (i.e., Requested Command to Abort Not Found) when the command to abort is not found.
*/
static uint32_t adm_cmd_abort(NVMEState *n, NVMECmd *cmd, NVMECQE *cqe)
{
	...
	if (c->sqid == ASQ_ID) {
		printf("Abort command for admin queue is not supported\n");
...
static uint32_t do_features(NVMEState *n, NVMECmd *cmd, NVMECQE *cqe)
{
	...
	case NVME_FEATURE_LBA_RANGE_TYPE:
		printf("NVME_FEATURE_LBA_RANGE_TYPE not supported yet\n");
		break;
